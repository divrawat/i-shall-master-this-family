
// export const DOMAIN = 'http://localhost:3000';
export const DOMAIN = 'https://www.ishallmasterthisfamily.in.net';


export const MANGA_DESCRIPTION0 = `Vinland Saga`;

export const MANGA_DESCRIPTION = `<a href="${DOMAIN}"><u>I Shall Master This Family</u></a> is a Korean Mahwa. The main protagnist Seong Jin-Woo, is driven by a relentless ambition to rise to the pinnacle of a formidable family hierarchy. Armed with cunning intelligence and a strategic mind, Seong Jin-Woo navigates a treacherous world of alliances and betrayals, forging unexpected bonds while facing formidable adversaries. `;

export const MANGA_NAME = 'I Shall Master This Family';
export const MANGA_AUTHOR = 'Mon';
export const MANGA_RELEASE = '2020';
export const MANGA_STATUS = 'Ongoing';
export const MANGA_ARTIST = 'Mon';
export const MANGA_STUDIO = 'None';
export const MANGA_GENRE = 'Mahwa, Comedy, Drama';

export const CHAPTER_PREFIX = 'i-shall-master-this-family-chapter';
export const IMAGE_PREFIX = 'images/i-shall-master-this-family';
export const NEXT_PREVIOUS_PREFIX = 'manga/i-shall-master-this-family-chapter';
export const URL_PREFIX = 'manga/i-shall-master-this-family-chapter';

export const AUTHOR_PAGE = `${DOMAIN}/i-shall-master-this-family-team`;
export const LOGO_URL = `${DOMAIN}/logo.webp`;
export const COVER_IMG = `${DOMAIN}/cover.webp`;
export const BEHIND_COVER_IMG = "https://pbs.twimg.com/media/F2uA6r4WsAAUO5f.jpg";

// https://cdn.imgchest.com/files/84jdcwx2984.png
// Navbar
export const logo = <img src={`${LOGO_URL}`} alt="Logo" className="h-[75px] w-[175px] mr-5 md:ml-0 ml-4 md:pb-2 md:my-0 my-1" />

export const APP_NAME = 'I Shall Master This Family';
export const NavbarName = "I Shall Master This Family";

export const APP_DESCRIPTION = `Read ${MANGA_NAME} manga online at ${DOMAIN}. Seong Jin-Woo, is driven by a relentless ambition to rise to the pinnacle of a formidable family hierarchy. Armed with cunning intelligence and a strategic mind, Seong Jin-Woo navigates a treacherous world of alliances and betrayals, forging unexpected bonds while facing formidable adversaries. `;

export const DISQUS_SHORTNAME = "my-cms-7";


export const MANGA_SUMMARY = [
    {
        id: 1,
        content: "I Shall Master This Family centers around Seong Jin-Woo, the determined protagonist who embarks on a quest to ascend to the leadership of a powerful and influential family. Driven by a deep-seated ambition and armed with exceptional intelligence and strategic acumen, Jin-Woo navigates a complex web of familial politics, rivalries, and alliances."
    },
    {
        id: 2,
        content: "From the outset, Jin-Woo faces formidable challenges as he contends with rivals within his own family and external adversaries seeking to undermine his ascent. His journey is marked by intricate power struggles, where every decision and alliance carries profound consequences for both himself and those around him."
    },
    {
        id: 3,
        content: "As Jin-Woo delves deeper into the secrets of his family's history, he uncovers long-buried truths and dark revelations that threaten to upend everything he knows. Each revelation not only tests his resolve but also forces him to confront his own inner demons and the weight of familial expectations."
    },
    {
        id: 4,
        content: "Throughout his journey, Jin-Woo forms unexpected alliances with unlikely allies who become pivotal in his quest for dominance. These relationships, fraught with tension and intrigue, add layers of complexity to his character as he navigates the murky waters of loyalty and betrayal."
    },
    {
        id: 5,
        content: "I Shall Master This Family is not just a story of ambition and power; it is a narrative that explores the complexities of identity, legacy, and the sacrifices one must make in pursuit of one's ambitions. It delves into the psychological and emotional toll of leadership, as Jin-Woo grapples with the responsibilities that come with wielding influence over his family and their vast empire."
    },
    {
        id: 6,
        content: "Ultimately, the series unfolds as a gripping saga of resilience, determination, and the enduring impact of familial bonds. It captivates readers with its intricate plot twists, dynamic character development, and a vivid portrayal of a world where ambition reigns supreme and the quest for power knows no bounds."
    }
];




export const navLinks = [
    { text: 'Home', href: `${DOMAIN}` },
    { text: 'DMCA', href: `${DOMAIN}/dmca` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` },
];

export const FooterLinks = [
    { text: 'About', href: `${DOMAIN}/about` },
    { text: 'Contact', href: `${DOMAIN}/contact` },
    { text: 'Disclaimer', href: `${DOMAIN}/disclaimer` },
    { text: 'Privacy Policy', href: `${DOMAIN}/privacy-policy` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` }
];



export const RelatedMangaLinks = [
    {
        imageUrl: `${DOMAIN}/related/whomademeaprincess.webp`,
        title: 'Who Made Me A Princess',
        link: 'https://www.whomademeaprincess.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/warriorhighschool.webp`,
        title: 'Warrior High School',
        link: 'https://www.warriorhighschool.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thislifestartsasachildactor.webp`,
        title: 'This Life Starts As a Child Actor',
        link: 'https://www.thislifestartsasachildactor.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thevillaninsfavouriteisme.webp`,
        title: 'The Villanins Favourite is Me',
        link: 'https://www.thevillaninsfavouriteisme.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thegreatestestatedeveloper.webp`,
        title: 'The Greatest Estate Developer',
        link: 'https://www.thegreatestestatedeveloper.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thedivorceesdessertcafe.webp`,
        title: 'The Divorcees Dessert Cafe',
        link: 'https://www.thedivorceesdessertcafe.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thebeginningaftertheend.webp`,
        title: 'The Beginning After The End',
        link: 'https://www.thebeginningaftertheend.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/talentedbabysquirrel.webp`,
        title: 'Talented Baby Squirrel',
        link: 'https://www.talentedbabysquirrel.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/solomaxlevelnewbie.webp`,
        title: 'Solo Max Level Newbie',
        link: 'https://www.solomaxlevelnewbie.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/secondliferanker.webp`,
        title: 'Second Life Ranker',
        link: 'https://www.secondliferanker.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/operationtruelove.webp`,
        title: 'Operation True Love',
        link: 'https://www.operationtruelove.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/omniscientreadersviewpoint.webp`,
        title: 'Omniscient Readers Viewpoint',
        link: 'https://www.omniscientreadersviewpoint.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/myschoollifepretendingtobeaworthlessperson.webp`,
        title: 'My School Life Pretending to be a Worthless Person',
        link: 'https://www.myschoollifepretendingtobeaworthlessperson.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/kallithechampion.webp`,
        title: 'Kalli The Champion',
        link: 'https://www.kallithechampion.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/ishallmasterthisfamily.webp`,
        title: 'I Shall Master This Family',
        link: 'https://www.ishallmasterthisfamily.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/iobtainedamythicitem.webp`,
        title: 'I Obtained A Mythic Item',
        link: 'https://www.iobtainedamythicitem.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/goldenlightgratiathechildlovedbygod.webp`,
        title: 'Golden Light Gratia The Child Loved By God',
        link: 'https://www.goldenlightgratiathechildlovedbygod.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/disastrouscatastrophicnecromancer.webp`,
        title: 'Disastrous Catastrophic Necromancer',
        link: 'https://www.disastrouscatastrophicnecromancer.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/adoptedbyamurderousdukefamily.webp`,
        title: 'Adopted by a Murderous Duke Family',
        link: 'https://www.adoptedbyamurderousdukefamily.in.net',
    },
];



export const chaptersData = [
    {
        "chapterNumber": "0",
        "numImages": 44
    },
    {
        "chapterNumber": "1",
        "numImages": 78
    },
    {
        "chapterNumber": "2",
        "numImages": 37
    },
    {
        "chapterNumber": "3",
        "numImages": 37
    },
    {
        "chapterNumber": "4",
        "numImages": 51
    },
    {
        "chapterNumber": "5",
        "numImages": 64
    },
    {
        "chapterNumber": "6",
        "numImages": 65
    },
    {
        "chapterNumber": "7",
        "numImages": 62
    },
    {
        "chapterNumber": "8",
        "numImages": 66
    },
    {
        "chapterNumber": "9",
        "numImages": 72
    },
    {
        "chapterNumber": "10",
        "numImages": 67
    },
    {
        "chapterNumber": "11",
        "numImages": 65
    },
    {
        "chapterNumber": "12",
        "numImages": 75
    },
    {
        "chapterNumber": "13",
        "numImages": 75
    },
    {
        "chapterNumber": "14",
        "numImages": 63
    },
    {
        "chapterNumber": "15",
        "numImages": 62
    },
    {
        "chapterNumber": "16",
        "numImages": 68
    },
    {
        "chapterNumber": "17",
        "numImages": 61
    },
    {
        "chapterNumber": "18",
        "numImages": 59
    },
    {
        "chapterNumber": "19",
        "numImages": 18
    },
    {
        "chapterNumber": "20",
        "numImages": 74
    },
    {
        "chapterNumber": "21",
        "numImages": 70
    },
    {
        "chapterNumber": "22",
        "numImages": 37
    },
    {
        "chapterNumber": "23",
        "numImages": 78
    },
    {
        "chapterNumber": "24",
        "numImages": 78
    },
    {
        "chapterNumber": "25",
        "numImages": 42
    },
    {
        "chapterNumber": "26",
        "numImages": 42
    },
    {
        "chapterNumber": "27",
        "numImages": 49
    },
    {
        "chapterNumber": "28",
        "numImages": 68
    },
    {
        "chapterNumber": "29",
        "numImages": 41
    },
    {
        "chapterNumber": "30",
        "numImages": 66
    },
    {
        "chapterNumber": "31",
        "numImages": 60
    },
    {
        "chapterNumber": "32",
        "numImages": 35
    },
    {
        "chapterNumber": "33",
        "numImages": 38
    },
    {
        "chapterNumber": "34",
        "numImages": 63
    },
    {
        "chapterNumber": "35",
        "numImages": 63
    },
    {
        "chapterNumber": "36",
        "numImages": 67
    },
    {
        "chapterNumber": "36a",
        "numImages": 17
    },
    {
        "chapterNumber": "36b",
        "numImages": 13
    },
    {
        "chapterNumber": "37",
        "numImages": 68
    },
    {
        "chapterNumber": "37a",
        "numImages": 9
    },
    {
        "chapterNumber": "38",
        "numImages": 69
    },
    {
        "chapterNumber": "38a",
        "numImages": 13
    },
    {
        "chapterNumber": "39",
        "numImages": 68
    },
    {
        "chapterNumber": "39a",
        "numImages": 14
    },
    {
        "chapterNumber": "40",
        "numImages": 46
    },
    {
        "chapterNumber": "41",
        "numImages": 77
    },
    {
        "chapterNumber": "41a",
        "numImages": 9
    },
    {
        "chapterNumber": "42",
        "numImages": 40
    },
    {
        "chapterNumber": "43",
        "numImages": 36
    },
    {
        "chapterNumber": "43a",
        "numImages": 5
    },
    {
        "chapterNumber": "44",
        "numImages": 45
    },
    {
        "chapterNumber": "44a",
        "numImages": 9
    },
    {
        "chapterNumber": "45",
        "numImages": 67
    },
    {
        "chapterNumber": "45a",
        "numImages": 8
    },
    {
        "chapterNumber": "46",
        "numImages": 58
    },
    {
        "chapterNumber": "47",
        "numImages": 63
    },
    {
        "chapterNumber": "47a",
        "numImages": 4
    },
    {
        "chapterNumber": "47b",
        "numImages": 5
    },
    {
        "chapterNumber": "48",
        "numImages": 81
    },
    {
        "chapterNumber": "49",
        "numImages": 76
    },
    {
        "chapterNumber": "50",
        "numImages": 66
    },
    {
        "chapterNumber": "51",
        "numImages": 83
    },
    {
        "chapterNumber": "52",
        "numImages": 98
    },
    {
        "chapterNumber": "53",
        "numImages": 74
    },
    {
        "chapterNumber": "54",
        "numImages": 88
    },
    {
        "chapterNumber": "55",
        "numImages": 84
    },
    {
        "chapterNumber": "56",
        "numImages": 37
    },
    {
        "chapterNumber": "57",
        "numImages": 85
    },
    {
        "chapterNumber": "58",
        "numImages": 83
    },
    {
        "chapterNumber": "59",
        "numImages": 83
    },
    {
        "chapterNumber": "60",
        "numImages": 69
    },
    {
        "chapterNumber": "61",
        "numImages": 41
    },
    {
        "chapterNumber": "62",
        "numImages": 89
    },
    {
        "chapterNumber": "63",
        "numImages": 82
    },
    {
        "chapterNumber": "64",
        "numImages": 37
    },
    {
        "chapterNumber": "65",
        "numImages": 102
    },
    {
        "chapterNumber": "66",
        "numImages": 98
    },
    {
        "chapterNumber": "67",
        "numImages": 109
    },
    {
        "chapterNumber": "68",
        "numImages": 105
    },
    {
        "chapterNumber": "69",
        "numImages": 98
    },
    {
        "chapterNumber": "70",
        "numImages": 92
    },
    {
        "chapterNumber": "71",
        "numImages": 92
    },
    {
        "chapterNumber": "72",
        "numImages": 88
    },
    {
        "chapterNumber": "73",
        "numImages": 105
    },
    {
        "chapterNumber": "74",
        "numImages": 41
    },
    {
        "chapterNumber": "75",
        "numImages": 91
    },
    {
        "chapterNumber": "76",
        "numImages": 92
    },
    {
        "chapterNumber": "77",
        "numImages": 36
    },
    {
        "chapterNumber": "78",
        "numImages": 84
    },
    {
        "chapterNumber": "79",
        "numImages": 82
    },
    {
        "chapterNumber": "79a",
        "numImages": 8
    },
    {
        "chapterNumber": "80",
        "numImages": 81
    },
    {
        "chapterNumber": "81",
        "numImages": 85
    },
    {
        "chapterNumber": "82",
        "numImages": 90
    },
    {
        "chapterNumber": "82a",
        "numImages": 13
    },
    {
        "chapterNumber": "83",
        "numImages": 84
    },
    {
        "chapterNumber": "84",
        "numImages": 84
    },
    {
        "chapterNumber": "85",
        "numImages": 35
    },
    {
        "chapterNumber": "86",
        "numImages": 81
    },
    {
        "chapterNumber": "87",
        "numImages": 37
    },
    {
        "chapterNumber": "88",
        "numImages": 37
    },
    {
        "chapterNumber": "89",
        "numImages": 78
    },
    {
        "chapterNumber": "90",
        "numImages": 84
    },
    {
        "chapterNumber": "91",
        "numImages": 37
    },
    {
        "chapterNumber": "92",
        "numImages": 82
    },
    {
        "chapterNumber": "93",
        "numImages": 89
    },
    {
        "chapterNumber": "94",
        "numImages": 82
    },
    {
        "chapterNumber": "95",
        "numImages": 80
    },
    {
        "chapterNumber": "96",
        "numImages": 35
    },
    {
        "chapterNumber": "97",
        "numImages": 84
    },
    {
        "chapterNumber": "98",
        "numImages": 83
    },
    {
        "chapterNumber": "99",
        "numImages": 89
    },
    {
        "chapterNumber": "100",
        "numImages": 41
    },
    {
        "chapterNumber": "101",
        "numImages": 79
    },
    {
        "chapterNumber": "102",
        "numImages": 79
    },
    {
        "chapterNumber": "103",
        "numImages": 35
    },
    {
        "chapterNumber": "104",
        "numImages": 77
    },
    {
        "chapterNumber": "105",
        "numImages": 36
    },
    {
        "chapterNumber": "106",
        "numImages": 75
    },
    {
        "chapterNumber": "107",
        "numImages": 82
    },
    {
        "chapterNumber": "108",
        "numImages": 81
    },
    {
        "chapterNumber": "109",
        "numImages": 82
    },
    {
        "chapterNumber": "110",
        "numImages": 38
    },
    {
        "chapterNumber": "111",
        "numImages": 84
    },
    {
        "chapterNumber": "112",
        "numImages": 41
    },
    {
        "chapterNumber": "113",
        "numImages": 34
    },
    {
        "chapterNumber": "114",
        "numImages": 35
    },
    {
        "chapterNumber": "115",
        "numImages": 86
    },
    {
        "chapterNumber": "116",
        "numImages": 97
    },
    {
        "chapterNumber": "117",
        "numImages": 74
    },
    {
        "chapterNumber": "118",
        "numImages": 99
    },
    {
        "chapterNumber": "119",
        "numImages": 20
    },
    {
        "chapterNumber": "120",
        "numImages": 21
    },
    {
        "chapterNumber": "121",
        "numImages": 13
    },
    {
        "chapterNumber": "122",
        "numImages": 14
    },
    {
        "chapterNumber": "123",
        "numImages": 12
    },
    {
        "chapterNumber": "124",
        "numImages": 7
    },
    {
        "chapterNumber": "125",
        "numImages": 17
    },
    {
        "chapterNumber": "126",
        "numImages": 7
    },
    {
        "chapterNumber": "127",
        "numImages": 6
    },
    {
        "chapterNumber": "128",
        "numImages": 14
    },
    {
        "chapterNumber": "129",
        "numImages": 17
    },
    {
        "chapterNumber": "130",
        "numImages": 7
    },
    {
        "chapterNumber": "131",
        "numImages": 17
    },
    {
        "chapterNumber": "132",
        "numImages": 6
    },
    {
        "chapterNumber": "133",
        "numImages": 7
    },
    {
        "chapterNumber": "134",
        "numImages": 7
    },
    {
        "chapterNumber": "135",
        "numImages": 12
    },
    {
        "chapterNumber": "136",
        "numImages": 16
    },
    {
        "chapterNumber": "137",
        "numImages": 14
    },
    {
        "chapterNumber": "138",
        "numImages": 6
    },
    {
        "chapterNumber": "139",
        "numImages": 6
    },
    {
        "chapterNumber": "140",
        "numImages": 7
    },
    {
        "chapterNumber": "141",
        "numImages": 6
    },
    {
        "chapterNumber": "142",
        "numImages": 5
    },
    {
        "chapterNumber": "143",
        "numImages": 21
    },
    {
        "chapterNumber": "144",
        "numImages": 13
    },
    {
        "chapterNumber": "145",
        "numImages": 11
    },
    {
        "chapterNumber": "146",
        "numImages": 5
    },
    {
        "chapterNumber": "147",
        "numImages": 31
    },
    {
        "chapterNumber": "148",
        "numImages": 10
    },
    {
        "chapterNumber": "149",
        "numImages": 15
    },
    {
        "chapterNumber": "150",
        "numImages": 12
    },
    {
        "chapterNumber": "151",
        "numImages": 12
    },
    {
        "chapterNumber": "152",
        "numImages": 15
    },
    {
        "chapterNumber": "153",
        "numImages": 11
    },
    {
        "chapterNumber": "154",
        "numImages": 13
    },
    {
        "chapterNumber": "155",
        "numImages": 14
    }
]